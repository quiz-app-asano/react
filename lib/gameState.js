...
class GameStateManager {
  ...
  async setCurrentQuestion(question) {
    ...
    console.log('Question started at:', new Date(questionStartTime).toLocaleTimeString(), 'for index:', this.data.questionIndex);
  ...
  async processPendingAnswers() {
    try {
      ...
      // 回答結果を記録（questionStartTimeは既にsetCurrentQuestionで設定済み）
      const currentResult = this.data.questionResults[this.data.questionIndex];
      if (currentResult) {
        Object.entries(this.data.pendingAnswers).forEach(([playerName, answerData]) => {
          if (answerData) {
